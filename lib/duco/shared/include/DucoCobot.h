...
namespace DucoRPC {
    ...
    struct OP {
		...
		char time_or_dist_3;       // 轨迹暂停或停止触发类型，0：不启用，1：时间触发。
		char trig_io_3;            // 轨迹触发控制柜IO的输出序号, 范围1-16
		bool trig_value_3;         // 轨迹触发控制柜IO的电平高低, false:低电平, true:高电平
		double trig_time_3;        // 当time_or_dist_3为1时, 当trig_time_3 >= 0时, 代表任务暂停或停止多少时间触发IO,单位: ms;
		double trig_dist_3;        // 不适用
    ...
    public:
         ...
         * @param p 对应末端的位姿, 位置单位: m, 姿态以Rx、Ry、Rz表示, 单位: rad
         * @param v 末端速度, 范围[0.01*PI/180, 1.25*PI]，单位: m/s
         * @param a 末端加速度, 范围[0.01*PI/180, 12.5*PI]，单位: m/s^2
         * @param r 融合半径, 单位: m, 默认值为 0, 表示无融合.当数值大于0时表示与下一条运动融合
         * @param q_near 目标点附近位置对应的关节角度, 用于确定逆运动学选解, 为空时使用当前位置
         * @param tool   设置使用的工具的名称, 为空时默认为当前使用的工具
         * @param wobj   设置使用的工件坐标系的名称, 为空时默认为当前使用的工件坐标系
