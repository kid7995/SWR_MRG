...
class AGP {
  ...
  public:
    ...
    void AGP_close() const;

    bool is_connected() const { return _connected; }

    void SetMode(MODE mode);
    void Control(FUNC Func);
    void SetSpeed(int16_t speed);
    void SetForce(int16_t force);
    void SetPos(int16_t pos);
    void SetTouchForce(int16_t Touchforce);
    void SetRampTime(int16_t RampTime);
    void SetLoadWeight(int16_t LoadWeight);
...
inline bool AGP::AGP_connect() {
    ...
    timeout.tv_sec = 100; // after 20 seconds connect() will timeout
    timeout.tv_usec = 0;
#endif
    ...
    if (!X_ISCONNECTSUCCEED(
            connect(_socket, (SOCKADDR *)&_server, sizeof(_server)))) {
 ...
 */
inline void AGP::AGP_close() const {
...
inline void AGP::SetMode(MODE mode) {
    if (!is_connected())
        return;
    modbus_write_register(5, mode);
}

inline void AGP::Control(FUNC Func) {
    if (!is_connected())
        return;
    modbus_write_register(1, Func);
}

/// <summary>
///
/// </summary>
/// <param name="Touchforce">unit : r/min </param>

inline void AGP::SetSpeed(int16_t speed) {
    if (!is_connected())
        return;
    modbus_write_register(2, speed);
}

/// <summary>
///
/// </summary>
/// <param name="Touchforce">unit : N </param>

inline void AGP::SetForce(int16_t force) {
    if (!is_connected())
        return;
    modbus_write_register(3, force);
}
/// <summary>
///
/// </summary>
...
///  unit : 0.01 mm
/// </param>
inline void AGP::SetPos(int16_t pos) {
    if (!is_connected())
        return;
    modbus_write_register(4, pos);
}
...
/// </summary>
/// <param name="Touchforce">unit : N </param>
inline void AGP::SetTouchForce(int16_t Touchforce) {
    if (!is_connected())
        return;
    modbus_write_register(6, Touchforce);
}

/// <summary>
///
/// </summary>
/// <param name="Touchforce">unit : ms </param>
inline void AGP::SetRampTime(int16_t RampTime) {
    if (!is_connected())
        return;
    modbus_write_register(7, RampTime);
}
...
/// </summary>
/// <param name="Touchforce">unit : N </param>
inline void AGP::SetLoadWeight(int16_t LoadWeight) {
    if (!is_connected())
        return;
    modbus_write_register(8, LoadWeight);
}

inline int16_t AGP::ReadStatus() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(21, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadSpeed() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(22, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadForce() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(23, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadPos() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(24, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadMode() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(25, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadErr() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(26, 1, &temp);
    return temp;
}

inline int16_t AGP::ReadTemp() {
    if (!is_connected())
        return -1;
    uint16_t temp;
    modbus_read_input_registers(27, 1, &temp);
    return temp;
}
 ...
 */
inline void AGP::modbus_build_request(uint8_t *to_send, uint16_t address,
                                      int func) const {
 ...
 */
inline int AGP::modbus_write(uint16_t address, uint16_t amount, int func,
                             const uint16_t *value) {
    int status = 0;
 ...
 * @param func      Modbus Functional Code
 */
inline int AGP::modbus_read(uint16_t address, uint16_t amount, int func) {
    uint8_t to_send[12];
    modbus_build_request(to_send, address, func);
    to_send[5] = 6;
    to_send[10] = (uint8_t)(amount >> 8u);
 ...
 */
inline int AGP::modbus_read_holding_registers(uint16_t address, uint16_t amount,
                                              uint16_t *buffer) {
    if (_connected) {
 ...
 * @param buffer      Buffer to Store Data Read from Registers
 */
inline int AGP::modbus_read_input_registers(uint16_t address, uint16_t amount,
                                            uint16_t *buffer) {
    if (_connected) {
 ...
 */
inline int AGP::modbus_read_coils(uint16_t address, uint16_t amount,
                                  bool *buffer) {
    if (_connected) {
        if (amount > 2040) {
            set_bad_input();
 ...
 */
inline int AGP::modbus_read_input_bits(uint16_t address, uint16_t amount,
                                       bool *buffer) {
    if (_connected) {
        if (amount > 2040) {
            set_bad_input();
 ...
 */
inline int AGP::modbus_write_coil(uint16_t address, const bool &to_write) {
    if (_connected) {
 ...
 */
inline int AGP::modbus_write_register(uint16_t address, const uint16_t &value) {
    if (_connected) {
 ...
 */
inline int AGP::modbus_write_coils(uint16_t address, uint16_t amount,
                                   const bool *value) {
    if (_connected) {
 ...
 */
inline int AGP::modbus_write_registers(uint16_t address, uint16_t amount,
                                       const uint16_t *value) {
    if (_connected) {
 ...
 */
inline ssize_t AGP::modbus_send(uint8_t *to_send, size_t length) {
    _msg_id++;
 ...
 */
inline ssize_t AGP::modbus_receive(uint8_t *buffer) const {
    return recv(_socket, (char *)buffer, MAX_MSG_LENGTH, 0);
}

inline void AGP::set_bad_con() {
    err = true;
    error_msg = "BAD CONNECTION";
}

inline void AGP::set_bad_input() {
    err = true;
 ...
 * @param func  Modbus Functional Code
 */
inline void AGP::modbuserror_handle(const uint8_t *msg, int func) {
    err = false;
    error_msg = "NO ERR";
    if (msg[7] == func + 0x80) {
        err = true;
